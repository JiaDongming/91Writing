...
91写作 是一个 AI 小说创作平台。前端是 Vue 3 + Element Plus SPA，SaaS 后端（Express + Prisma + PostgreSQL）正在 `dev` 分支上开发中。
...
# 后端（server/）
npm --prefix server run dev              # Express 开发模式，tsx watch，端口 4000
npm --prefix server run build            # TypeScript 编译
npm --prefix server run prisma:generate  # 生成 Prisma client
npm --prefix server run prisma:push      # 推送 schema 到数据库
npm --prefix server run prisma:seed      # 写入初始种子数据
...
来自 `AGENTS.md`：
- 新功能开发默认使用 `using-superpowers` 流程，优先使用已安装的 skill 而非跳过流程直接写代码
...
- **纯前端模式（当前）：** 用户在界面右上角「API配置」中填写 Key → 存储在 `localStorage` 的 `apiConfigType`、`officialApiConfig`、`customApiConfig` 键中
- **后端模式：** 将 `server/.env.example` 复制为 `server/.env`，配置 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL`、`DATABASE_URL`、`JWT_SECRET`
